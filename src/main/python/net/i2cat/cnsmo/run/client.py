...
def get_app_request(host, port, service_id):
    ...
    d = dict(service_id=service_id,
             ...
             endpoints=[{"uri":"http://%s:%s/vpn/client/config/" %(host, port), "driver":"REST", "logic":"upload", "name":"set_config"},
                        {"uri":"http://%s:%s/vpn/client/cert/ca/" %(host, port), "driver":"REST", "logic":"upload", "name":"set_ca"},
                        ...
                        {"uri":"http://%s:%s/vpn/client/start/" %(host, port),  "driver":"REST", "logic":"post", "name":"start"},
                        {"uri":"http://%s:%s/vpn/server/stop/" %(host, port), "driver":"REST", "logic":"post", "name":"stop"},])
    return d
...
def main(host, port, redis_address, service_id):
    ...
    configurer = CNSMOManager(redis_address, service_id, "ClientVPN", bash_deployer, None)
    configurer.start()
