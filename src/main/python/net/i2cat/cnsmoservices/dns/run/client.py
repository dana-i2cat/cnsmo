...
def get_app_request(host, port, service_id):
    ...
    d = dict(service_id=service_id,
             ...
             resources=["https://raw.githubusercontent.com/dana-i2cat/cnsmo/%s/src/main/python/net/i2cat/cnsmoservices/dns/app/client.py" % gitBranch,
                        ],
...
def main(host, port, redis_address, service_id):
    ...
    configurer = CNSMOManager(redis_address, service_id, "DNSClient", bash_deployer, None)
    configurer.start()
