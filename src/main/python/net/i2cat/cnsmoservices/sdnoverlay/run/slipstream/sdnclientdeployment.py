...
def configure_bridge(NIC, IP, GW, MAC, MASK):
    ...
    err = call("sudo ovs-vsctl set bridge br-ext protocols=%s" %(protocols))
    totalErr = totalErr + check_error(err)
    logger.debug("Done!")
...
def subscribe_to_controller(PROTO_SDN,SDN_CTRL_IP_PORT):
    ...
    totalErr = totalErr + check_error(err)
    err = call("sudo echo \"ovs-ofctl add-flow br-ext \\\"in_port=1, tcp, tp_dst=5001, priority=600, actions=drop\\\" > /dev/null 2>&1\" >> ./temp2.sh")
    totalErr = totalErr + check_error(err)
    '''
    ...
    totalErr = totalErr + check_error(err)
    err = call("sudo echo \"ovs-ofctl add-flow br-ext \\\"in_port=1, tcp, tp_dst=6640, priority=600, actions:output=LOCAL\\\" > /dev/null 2>&1\" >> ./temp2.sh")
    totalErr = totalErr + check_error(err)
    err = call("sudo echo \"ovs-ofctl add-flow br-ext \\\"in_port=1, tcp, tp_dst=22, priority=600, actions:output=LOCAL\\\" > /dev/null 2>&1\" >> ./temp2.sh")
    totalErr = totalErr + check_error(err)
    err = call("sudo echo \"ovs-ofctl add-flow br-ext \\\"in_port=1, tcp, tp_dst=20091, priority=600, actions:output=LOCAL\\\" > /dev/null 2>&1\" >> ./temp2.sh")
    totalErr = totalErr + check_error(err)
    err = call("sudo echo \"ovs-ofctl add-flow br-ext \\\"in_port=1, tcp, tp_dst=20379, priority=600, actions:output=LOCAL\\\" > /dev/null 2>&1\" >> ./temp2.sh")
    totalErr = totalErr + check_error(err)
    ...
    totalErr = totalErr + check_error(err)
    err = call("sudo echo \"ovs-ofctl add-flow br-ext \\\"in_port=1, udp, tp_dst=9004, priority=600, actions:output=LOCAL\\\" > /dev/null 2>&1\" >> ./temp2.sh")
    totalErr = totalErr + check_error(err)
